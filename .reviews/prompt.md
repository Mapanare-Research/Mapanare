...
Create an agent team of 7 reviewers to perform a deep, comprehensive code review of the Mapanare programming language codebase. Mapanare is an AI-native compiled programming language where agents, signals, streams, and tensors are first-class primitives. It compiles to native binaries via LLVM (primary), C (fallback), and WebAssembly (browser/server). A self-hosted compiler exists (15,000+ lines of .mn). Python and PHP transpilers allow `mapanare compile main.py` and `mapanare compile app.php`.

This is version `v3.25.0` — the last version before v4.0.0 (production release). Since v3.14.0 (last reviewed), 11 versions shipped: drop glue + arenas (v3.17-3.18), self-hosted While/Break/Continue (v3.19), arithmetic traits + optimizer convergence (v3.20), DX polish (v3.21), structural sharing (v3.22), dynamic `any` type (v3.23), Python transpiler (v3.24), PHP transpiler (v3.25). Reviewers should hold the codebase to RELEASE-QUALITY standards. v4.0.0 is a pure quality gate — this review determines what must be fixed before shipping.
...
```markdown
# [Reviewer Name] -- [Language/Domain] Review of Mapanare v3.25.0
**Reviewer:** [Name]
...
The `README.md` in `.reviews/v3.25.0/` must contain:

```markdown
# Mapanare v3.25.0 -- Code Review Summary
...
- **Output:** `.reviews/v3.25.0/01-viper.md`
...
- **Output:** `.reviews/v3.25.0/02-boa.md`
...
- **Output:** `.reviews/v3.25.0/03-cobra.md`
...
- **Output:** `.reviews/v3.25.0/04-mamba.md`
...
- **Output:** `.reviews/v3.25.0/05-anaconda.md`
...
- **Output:** `.reviews/v3.25.0/06-rattler.md`
...
- **Output:** `.reviews/v3.25.0/07-coral.md`
...
1. Create the `.reviews/v3.25.0/` directory
...
4. Compile `.reviews/v3.25.0/README.md` with the consensus table, cross-referenced issues, and prioritized action items
...
- Python and PHP transpilers (`from_python.py`, `from_php.py`) compile .py/.php to native via Mapanare
- Self-hosted compiler: 15,000+ lines of .mn across 11 modules, compiles itself
- C runtime: arena allocator, lock-free ring buffers, thread pool, agent scheduler, string interning
- Dynamic `any` type (v3.23.0): MnValue tagged union for gradual typing
- Drop glue: strings, closures, lists, maps, signals, streams all freed on function exit
- Per-function arenas for non-escaping temporaries (conservative escape analysis)
- This is v3.25.0 — the LAST version before v4.0.0 production release. Hold to release-quality standards
- Previous review was v3.14.0 — reviewers MUST read `.reviews/v3.14.0/README.md` and assess what was fixed
...
1. Change `v3.25.0` to the new version tag everywhere in this file (search and replace)
