...
| 49 | Drop-glue skip-struct-ret early return at `emit_llvm_text.py:1097-1099` | v4.18.0 era | LOW | **8** | OPEN | v4.62.0+ (Arc 7). v4.60.0 reconciliation: still open, re-tracked from v4.33.0. Viper V1. |
| 50 | Agent `mapanare_agent_destroy` drops in-flight messages without freeing them | v4.26.0 | LOW | **2** | OPEN | v4.62.0+ (Arc 7). v4.60.0 reconciliation: still open, re-tracked from v4.33.0. 20-line runtime fix. Viper M5. |
...
| P2 | `pattern_matching.py` zero dedicated unit tests | v4.36.0 (Boa M1, Anaconda) | MEDIUM | **2** | OPEN | v4.62.0+ (Arc 7). v4.60.0 reconciliation: still open, re-tracked from v4.37.0. |
| P3 | Self-hosted guard fall-through divergence (jump-to-next vs decision-tree rebuild) | v4.36.0 (Cobra, Rattler) | MEDIUM | **2** | OPEN | v4.62.0+ (Arc 7). v4.60.0 reconciliation: latent for current test corpus. Re-tracked from v4.37.0. |
...
| P6 | Unreachable-arm warning path zero test coverage | v4.36.0 (Boa M2) | MEDIUM | **2** | OPEN | v4.62.0+ (Arc 7). v4.60.0 reconciliation: re-tracked from v4.37.0. |
| A10b | Self-hosted const scope issue: const symbols not found in fn bodies | v4.55.0 | LOW | **3** | OPEN | v4.62.0+ (Arc 7). scope-chain threading issue in self-hosted semantic.mn; Python pipeline works. |
