...
- **v5.3.1** (shipped) — **Quick-win closeout — lint GREEN, stream-C fixed, docs accurate.** First release of the v5.3.x closeout arc targeting v5.4.0 re-panel at 9.5+. Closes 5 MEDIUM + 3 LOW carry-forwards from v5.3.0 panel: Lint-v5.2.0 (green), Bo.15 (README fixed-point claim qualified), Bo.16 (known_issues.md updated for v5.2.0 registry), Stream-C (4 stream tests fixed — `MnList list = {0}` → `__mn_list_new(sizeof(int64_t))`; all 3 C hardening classes pass), An.9r (unified-return test accepts 0 or 1 switches for LLVM 18), Bo.17 (zh-CN/pt/es badges bumped), Bo.14r (getting_started.md refreshed). No compiler or runtime source changes. 5462 passed / 2 failed (version-mismatch only). Expected panel impact: Anaconda +0.3-0.5, Boa +0.1-0.2, Mamba +0.05. See `docs/roadmap/v5/v5.3.1/`.
- **v5.3.2** (shipped) — **In.1-stage2 — restore fixed-point (clone_instr_for_inline).** Extends `clone_instr_for_inline` in `mapanare/self/mir_opt.mn` from 10 to all 38 Instruction enum variants. The v5.1.2 In.1 fix handled use-renaming for all variants but only cloned definitions for 10 — the remaining 28 fell through with un-renamed destinations, causing `llvm-as` rejection of stage2.ll. Three additional fixes: (1) `rename_phi_pred` helper renames PHI predecessor labels after block splitting, (2) param-count guard skips inlining when `len(call_args) != len(callee.params)` (handles `new_decorator` name collision between ast.mn/parser.mn), (3) downstream PHI predecessor labels updated from original block to merge block. Fixed-point: **BROKEN → stage2 llvm-as OK**. stage3 blocked by MIR verifier in stage2 binary (Ve.1 LOW — bootstrapping divergence, empty `expr_kind` match arms). 54/66 goldens unchanged. 9 inline rename tests (5 new: FieldGet, StructInit, IndexGet, EnumTag/Payload, param-count mismatch). Closes In.1-stage2 MEDIUM. Opens Ve.1 LOW. See `docs/roadmap/v5/v5.3.2/`.
- **v5.3.0** (shipped) — **THE PANEL — 9.30/10, Option A.** Seven-reviewer panel grading v5.0.1-v5.2.0 arc (12 releases, 19 carry-forward closures). Aggregate 9.30, 5 EXCEEDS / 2 MEETS / 0 NEEDS WORK. Per-reviewer (v4.154.0 → v5.2.0): Rattler 9.6→9.3 (-0.3), Viper 9.6→9.7 (+0.1), Anaconda 9.4→8.9 (-0.5), Cobra 9.1→8.8 (-0.3), Coral 9.3→9.4 (+0.1), Boa 9.3→9.4 (+0.1), Mamba 9.3→9.6 (+0.3). Dip from 9.37 due to In.1 fixed-point regression (Cobra/Rattler) and v5.2.0 lint gap (Anaconda). No compiler or runtime source changes. Evidence: `docs/roadmap/v5/v5.3.0/MEASUREMENTS.md`. Reviews: `.reviews/v5.2.0/`. See `docs/roadmap/v5/v5.3.0/`.
- **v5.2.0** (shipped) — **Package Registry MVP — publish + install end-to-end.** First minor bump since v5.0.0 with user-facing feature. Closes last open V5_READINESS.md item. Backend: `mapanare.dev/api/packages` (FastAPI + PostgreSQL, existing `mapanare-website` infra). GitHub OAuth auth; API tokens; download tracking. Team-only publishing for MVP. Client: `mapanare install foo@1.2.3` downloads + SHA-256 verifies + extracts to `mn_modules/foo-1.2.3/`; `mapanare install` (no args) resolves `mapanare.toml` deps; `mapanare publish` uploads with metadata; `mapanare.lock` deterministic. Install dir `mapanare_packages/` → `mn_modules/`. 51 tests in `tests/registry/`. `docs/guides/packages.md`. No install-time scripts (supply-chain discipline). Deferred: open publishing, SAT resolver, yanking, vendoring (v5.3+). See `docs/roadmap/v5/v5.2.0/`.
- **v5.1.4** (shipped) — **Perf.2 — lazy thread creation in coro scheduler.** `__mn_coro_scheduler_init` pre-creates 2 workers eagerly, grows pool lazily when `active_tasks > workers * 8`. Idle workers self-exit after 100 ms (floor: 2 workers). Race-safe teardown via `spawned[]` + `worker_exited[]` arrays. Default-settings async geomean: 2.3 → 1.19 ms (**0.91× Go without env var**). `MAPANARE_ASYNC_THREADS` preserved as optional override. TSan 0 races; valgrind 0 leaks. 54/66 goldens. PARITY_GAPS.md: Perf.2 → Historical. Closes Mamba v4.154.0 "not a portable claim". See `docs/roadmap/v5/v5.1.4/`.
- **v5.1.3** (shipped) — **Own.1 Phase 1 — compiler-enforced drop-glue skip on ownership transfer.** Applies Cb.7 zero-after-push workaround to `register_struct` (lower.mn:330-336) and `register_enum` (lower.mn:364-369) — the two sites Viper flagged every panel since v4.99.0 (28 releases open). Design finding: self-hosted emitter has NO `emit_drop_glue` — no epilogue cleanup, no ownership tracking. Python emitter already safe via `_do_call` blanket move (line 3882). Full Move instruction + `@takes_ownership` + `moved_locals` infrastructure deferred to v5.1.4+ (requires drop-glue emission in self-hosted emitter). DESIGN.md documents annotation-vs-inference decision. Valgrind 0 ERRORS (66 goldens). 54/66 goldens. PARITY_GAPS.md: Own.1 Phase 1 → Historical. See `docs/roadmap/v5/v5.1.3/`.
- **v5.0.6** (shipped) — **Multi-cycle hygiene closeout — 8 items, zero compiler-semantic changes.** The "errors dragged from earlier versions" release. Bundles eight carry-forwards flagged 2-3 cycles ago: **Bo.12-table** (README benchmark table now shows v4.153.0 numbers — 168× Py / 0.85× Go / 1.17× Rust / 0.96× C), **Bo.12-i18n** (`docs/README.{es,pt,zh-CN}.md` version badges 5.0.0 → 5.0.6, test badges 5534+ → 5720+), **Rt.4** (`mapanare/self/emit_llvm.mn::llvm_type_size` returned hardcoded 16 for every `%enum.*`; Rt.1 v4.124.0 made 2-slot inline enums `{i64,i64,i64}` = 24B, so the comment became actively false and the 16 a latent heap overflow once Cb.15 v5.0.4 wired self-hosted sret classification — now returns 24 safe upper bound with three-layout comment), **Bn.3** (`benchmarks/cross_language/run_benchmarks.py` reads live VERSION; third Mamba cycle closed), **Cb.6-test** (new `tests/llvm/test_enum_inline_parity.py` regression gate for typed-pointer `ends_with("*")` rejection in `type_fits_inline_slot`), **An.9** (new `tests/llvm/test_unified_return_shape.py` IR-shape gate for E1 unified-return — single switch in `@area` pre-opt; sret on `@make_shape`; post-O2 single-switch when `opt` available), **An.10** (new `scripts/count_tests.py` + `make count-tests` — 4209 `def test_*` declarations expanding to ~5720 pytest-collected; deterministic delta source), **Dr.1-mutation** (`scripts/build_stage1.py` uses `tempfile.TemporaryDirectory` for version-placeholder substitution; source tree no longer mutated during build — the v4.139.0 try/finally restore-path fragility pattern ends). Fixed-point, 54/66 goldens, sanitizer results all unchanged (no compiler source semantic changes). PARITY_GAPS.md: 8 items move to Historical; the multi-cycle-carry-forward class Cobra flagged at v4.154.0 is now caught up. See `docs/roadmap/v5/v5.0.6/`.
...
- **v5.0.0** (shipped) — **First clean v5 tag.** The 136-release v4.x arc closes. Tagged at commit `0c6ec42` after v4.160.0 polish (macOS native binary, Windows CLI bundle stabilisation). `v5.0.0-rc1` (tagged at v4.136.0 panel) transitioned to clean `v5.0.0`. Shipped assets: `mapanare-{linux,mac,win}-x64.{tar.gz,zip}` CLI bundles (all three platforms) + `mnc-{linux-x64,darwin-arm64}` native binaries (Linux + macOS only). Windows CLI bundle (`mapanare-win-x64.zip`) includes vendored w64devkit MinGW + prebuilt `libmapanare_rt.a` so `mapanare run/build` works without user-installed gcc (commits `7158473` + `5c99b11`).
