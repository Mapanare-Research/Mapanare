...
- **v5.2.0** (shipped) — **Package Registry MVP — publish + install end-to-end.** First minor bump since v5.0.0 with user-facing feature. Closes last open V5_READINESS.md item. Backend: `registry.mapanare.dev` (Cloudflare Workers + R2), HTTP API at `/v1/packages*`. GitHub OAuth auth; KV tokens; IP rate limiting. Team-only publishing for MVP. Client: `mapanare install foo@1.2.3` downloads + SHA-256 verifies + extracts to `mn_modules/foo-1.2.3/`; `mapanare install` (no args) resolves `mapanare.toml` deps; `mapanare publish` uploads with metadata; `mapanare.lock` deterministic. Install dir `mapanare_packages/` → `mn_modules/`. API paths `/api/packages` → `/v1/packages`. 51 tests in `tests/registry/`. `docs/guides/packages.md`. No install-time scripts (supply-chain discipline). Deferred: open publishing, SAT resolver, yanking, vendoring (v5.3+). See `docs/roadmap/v5/v5.2.0/`.
