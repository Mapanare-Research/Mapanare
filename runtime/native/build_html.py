"""Build the Mapanare HTML, timing, and URL runtime shared library.

Compiles mapanare_html.c into a loadable shared library for HTML parsing
(lexbor via dlopen), timing primitives, environment variable access, and
URL parsing.
"""
...
SRC = NATIVE_DIR / "mapanare_html.c"
...
def _lib_name() -> str:
    ...
    if platform.system() == "Windows":
        return "mapanare_html.dll"
    elif platform.system() == "Darwin":
        return "libmapanare_html.dylib"
    else:
        return "libmapanare_html.so"
...
def build() -> Path:
    """Compile the HTML runtime and return the path to the shared library."""
    out = lib_path()
...
def _build_msvc(out: Path) -> None:
    ...
    for ext in [".obj", ".lib", ".exp"]:
        p = NATIVE_DIR / f"mapanare_html{ext}"
        if p.exists():
            p.unlink()
