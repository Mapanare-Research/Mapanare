...
static const char PTX_TENSOR_ADD[] =
    ...
    "    .param .u64 param_n\n"
    ") {\n"
    "    .reg .u64  %ra, %rb, %rout, %rn, %ridx, %roff, %tmp;\n"
    "    .reg .u32  %t, %b, %d;\n"
    "    .reg .f64  %fa, %fb, %fr;\n"
    "    .reg .pred %p;\n"
    "\n"
    ...
    "    mov.u32       %b,    %ctaid.x;\n"
    "    mov.u32       %d,    %ntid.x;\n"
    "    mul.wide.u32  %tmp,  %b, %d;\n"
    "    cvt.u64.u32   %ridx, %t;\n"
    "    add.u64       %ridx, %tmp, %ridx;\n"
    "\n"
...
static const char PTX_TENSOR_MATMUL[] =
    ...
    "    mov.u32       %bx,   %ctaid.x;\n"
    "    mov.u32       %dx,   %ntid.x;\n"
    "    mul.wide.u32  %c_tmp, %bx, %dx;\n"
    "    cvt.u64.u32   %col,  %tx;\n"
    "    add.u64       %col,  %c_tmp, %col;\n"
    "\n"
    "    mov.u32       %ty,   %tid.y;\n"
    "    mov.u32       %by,   %ctaid.y;\n"
    "    mov.u32       %dy,   %ntid.y;\n"
    "    mul.wide.u32  %r_tmp, %by, %dy;\n"
    "    cvt.u64.u32   %row,  %ty;\n"
    "    add.u64       %row,  %r_tmp, %row;\n"
    "\n"
