...
## Where We Are (v4.113.0 **Phase D release 3 — last v4.99.0 docket items closed.** Closes the three remaining items from the v4.99.0 panel (#8 coroutine frame layout coupling, #10 keyword-collision SPEC, #11 async error messages); zero open items from that panel after this release. **Docket #8**: `mn_coro_is_done` / `mn_coro_resume` in `runtime/native/mapanare_runtime.c` replaced raw `*(void**)handle` casts with a named `mn_coro_frame_prefix_t` struct documenting the LLVM switched-resume ABI (resume_fn offset 0, destroy_fn offset sizeof(void*)). Behaviourally equivalent, grep-able, one named definition to update if the ABI ever moves. Pre-change byte-for-byte valgrind control proves the change is memory-neutral; all 3 async goldens still produce 42/43/110. **Docket #10**: new `docs/SPEC.md` §2.1.1 "Reserved Keyword Master List" publishes all 42 hard-reserved identifiers with English/Spanish/category/AST role, cross-referenced to `mapanare.lark:380-427` and `self/lexer.mn:59-177`. Removed stale "async/await soft-reserved" note. **Docket #11**: 5 async failure sites in `mapanare_runtime.c` replaced silent-drop / NULL-deref with specific stderr + exit(1) — worker pthread_create failure (names index + strerror), scheduler-not-initialised guard, deque+overflow both full, await with overflow full (handle + Future address), file_read_async allocation paths. **Golden tests: 26/64 preserved** (identical to v4.112.0); stage2 0/11 unchanged. **Next: v4.114.0 is the Phase D panel — 7 reviewers grade v4.111.0-v4.113.0.**)
