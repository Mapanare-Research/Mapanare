...
## Where We Are (v4.117.0 **Phase E release 3 — testing sweep: sanitizer CI, flaky audit, coverage.** Zero compiler/runtime code changes. Makes the test infrastructure production-grade before v4.120.0 panel. ASan + TSan CI gates (permanent since v4.105.0) extended with v4.115.0 async I/O demos in tsan-async job. **Flaky audit (tests/FLAKY_AUDIT.md)**: pytest ran 5x across 1,501 tests, pairwise-diff of failure sets empty — **zero flaky tests**; 22 observed failures are deterministic pre-existing bugs catalogued by bucket. **Coverage (tests/COVERAGE.md)**: 43% aggregate, 73% within the core pipeline (parser 78%, semantic 81%, lower 69%, emit_llvm_text 65%, mir_opt 72%, mir 95%, types 92%, ast_nodes 100%); below-50% tail identified with reasons; 5 recommendations for future coverage work. **Integration pipeline hardening (tests/integration/test_pipeline_hardening.py, 6 new tests, all PASS)**: deliberately broken inputs assert the harness fails loudly at the correct stage — unparseable source → emit, invalid IR → llvm-as, 42-exit binary → nonzero captured, sleep(60) → TimeoutExpired, stdout mismatch → reported, hello.mn happy path still passes. New informational coverage CI job; flips to enforcing after 5 stable releases. `libmapanare_rt.a` byte-identical to v4.116.0. **Next: Phase F opens at v4.118.0 — final cross-language benchmark with all Phase A-E fixes landed.**)
