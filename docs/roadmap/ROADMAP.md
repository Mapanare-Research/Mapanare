...
## Where We Are (v4.112.0 **Phase D release 2 — fixed-point verification + docket #7 closed.** Ran the 3-stage fixed-point script, classified divergences across 4 categories (byref / structural / cosmetic / semantic-gap), closed docket #7 (byref size heuristic) via a single-file fix in `mapanare/self/emit_llvm.mn`: new `struct_byte_size(st, ty)` resolves `%struct.Foo` through the registered struct table; new `is_byref_type_st(st, ty)` uses real sizes matching Python bootstrap's `_tsz` algorithm. All 7 call sites updated. 16-byte structs like `Small`, `Point`, `Counter` now correctly pass by value instead of being forced to byref by the old 256-byte stub. **Golden tests: 26/64 preserved (zero regressions).** **Fixed-point convergence not measured** — stage1 can't self-compile because self-hosted `semantic.mn` doesn't register `None` as a constructor (pre-existing gap surfaced in v4.111.0's stage2 validation; Python bootstrap bypasses via `skip_check=True`). New docket **Sh.8** opened for self-hosted None/Some/Ok registration. Docket Sh.3 CLOSED. Published `DIVERGENCE_ANALYSIS.md`. 6/10 exit criteria green. **Next: v4.113.0 closes remaining v4.99.0 panel items (#8 coroutine frame, #10 keyword collision SPEC, #11 async errors); v4.114.0 is the Phase D panel.**)
