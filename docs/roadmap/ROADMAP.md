...
## Where We Are (v4.115.0 **Phase E release 1 — async I/O demo running natively.** v4.114.1 patch skipped per user direction; Phase D findings carry into Phase E as open items. Two new example programs close the v4.99.0 panel's "no async program has been demonstrated with real I/O" gap: **`examples/async_file_io.mn`** (cooperative async file pipeline — read input, count lines+words via `byte_at`, write two-field summary file from inside `await write_summary`, verified at `-O0` and `-O2`), and **`examples/async_http_demo.mn`** (real HTTP GET to `http://example.com/`, async pipeline over the 540-byte response body, summary file on disk). New **`docs/guides/async.md`** walks mental model, syntax, both examples, what-works / what-doesn't tables with docket IDs, and Sh.9 workaround recipes. **Zero compiler/runtime/self-hosted code changes** (Phase 4 confirmed no new C runtime symbols needed — `libmapanare_rt.a` byte-identical to v4.114.0). Two Python-bootstrap emitter bugs surfaced and worked around in both examples: **Sh.9a** (`await` on String-returning async fn produces invalid IR) and **Sh.9b** (DCE eliminates unused-return awaits, dropping side effects). Both documented as recipes in the guide so users don't re-hit them. **Sh.10** opened for making `__mn_file_read_async` user-callable (pre-requisite: Sh.9a). **Regression clean**: Python-bootstrap 63/64, async goldens 55/56/57 → 42/43/110 unchanged. **Next: v4.116.0 is the documentation batch — README refresh against Phase C geomeans, SPEC sync, cookbook pass, getting-started guide.**)
