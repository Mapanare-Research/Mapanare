...
## Where We Are (v5.2.0 **Package Registry MVP — publish + install end-to-end.** First minor bump since v5.0.0 with a user-facing feature. Closes the last open item on V5_READINESS.md ("package registry infrastructure"). **Backend**: `registry.mapanare.dev` — Cloudflare Workers + R2, HTTP API at `/v1/packages*` (list, get, version, download tarball, publish, search). GitHub OAuth for auth; KV for tokens; IP rate limiting via Workers binding. Team-only publishing for MVP (`Mapanare-Research` org members); open publishing v5.3+. **Client** (`stdlib/pkg.py` + `mapanare/cli.py`): `mapanare.toml` gains `repository` field; `mapanare install foo@1.2.3` downloads from registry, verifies SHA-256, extracts to `mn_modules/foo-1.2.3/`; `mapanare install` (no args) resolves all `[dependencies]` from `mapanare.toml`; `mapanare publish` uploads tarball with metadata fields to `/v1/packages`; `mapanare.lock` records resolved versions deterministically. Install directory renamed `mapanare_packages/` → `mn_modules/` with versioned subdirectories. API paths migrated `/api/packages` → `/v1/packages`. Semver resolution: latest-satisfying with `^`, `~`, `>=`, exact, `*` — no SAT solver. **No install-time scripts allowed** (supply-chain discipline). **Tests**: `tests/registry/` — 51 tests covering manifest parsing, lockfile round-trip, semver resolution, tarball creation, integrity hashing, install-all-from-manifest, and registry URL/path assertions. **Docs**: `docs/guides/packages.md` — user-facing guide for publish, install, `mapanare.toml` schema, lockfile, auth flow. **Deferred**: open publishing, peer-dep resolution, yanking, vendoring (all v5.3+ scope). **Next: v5.3.0.**)
