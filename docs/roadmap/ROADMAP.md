...
## Where We Are (v4.122.0 **Phase F closeout release 2 — Qs.1 fix (`List<Int>` indexing in argument position).** The v4.120.0 panel's flagship correctness docket is closed. Single 6-line edit in `mapanare/lower.py::MIRLowerer._lower_let`: after the existing empty-list annotation block patches `ListInit.elem_type`, also rebind `val = Value(name=val.name, ty=declared)` so the named alias (`%arr`) carries the full list element type. Before the fix, `let arr: List<Int> = []; print(str(arr[0]))` printed `<?>` on the native pipeline (the LLVM emitter saw UNKNOWN-typed list elements and fell through to the `<?>` placeholder) and `let v: Int = arr[0]; print(str(v))` bound a raw heap pointer ptrtoint'd to i64. Python bootstrap interpreter produced correct output all along — the bug was isolated to the MIR → LLVM emitter path. Self-hosted compiler structurally doesn't have this bug (verified empirically: v4.121.0 `mnc-stage1` already produces the correct `42 / 42 / 99 / 100 / 141` output for the new golden). **New regression surface**: `tests/golden/65_list_int_indexing.mn` (5 indexing patterns through the full pipeline) + 5 IR-level tests in `tests/llvm/test_emitter_hardening.py::TestListIntIndexingQs1` pinning the invariant (List<Int> emits `load i64, ptr`; List<Float> emits `load double, ptr`; List<MyStruct> still loads struct aggregate; no `ptrtoint` in let-binding; no `<?>` in str() argument). **Golden 27/65** through mnc-stage1 (up from 26/64 at v4.121.0; zero regressions). **Full pytest 38 failures**, all pre-existing An.1 carry-forward — confirmed by stash-diff receipt showing the only delta vs v4.121.0 HEAD is the new 65 golden integration test passing post-fix. `libmapanare_rt.a` byte-identical to v4.121.0. **No new dockets; closes Qs.1** — V5_READINESS had flagged it "would embarrass a v5 label." **Next: v4.123.0 dead-code sweep** — delete `optimizer.py` (1,203 lines, 9% coverage via `--legacy-optimizer` flag that zero tests use) and remove the TBAA metadata declaration block confirmed 100% dead by v4.109.0 forensics. Pure cleanup, net negative lines.)
