...
## Where We Are (v4.109.0 **Phase C release 3 shipped — Arcs 11–12 optimizer ROI forensics.** Pure investigation, zero code changes. `TOTAL_RESULTS.md`'s 0.992× geomean at -O2 hid a heterogeneous mix: matmul_naive **+24%** (real Arc 11 win), quicksort +9%, fib 0%, string_concat **−21%** (Arc 11 hints actively hurt after v4.108.0's StringBuilder pass — `willreturn` on `__mn_sb_*` declarations blocks DSE). Three discoveries reshape the Arc-11 story: (1) **TBAA metadata is 100% dead** — defined in module header but never attached to any load/store across all 4 benchmarks; Arc 11 work contributed nothing to alias analysis. `emit_llvm_text.py:913` has a comment describing intended wiring that was never written. (2) **Function attributes on runtime-call declarations** (`nounwind`, `willreturn`, `readonly`) are the load-bearing Arc 11 contribution — they cross pass boundaries via LLVM's attribute table. (3) **Inline `nsw`/`nuw` flags are mostly redundant** — LLVM independently infers all 13 `nuw` on matmul post-O2 even without frontend seeding. H2 rejected for fib: scaling to fib(45) (120× work) collapses the small-sample delta to noise. H3 subtly confirmed: zero instruction-level differences from any single LLVM pass on hinted vs stripped IR; matmul's 24% speedup comes from pass-ordering interactions, not inline hint consumption. Published `benchmarks/optimizer/OPT_ROI_ANALYSIS.md` (264 lines). New dockets: TBAA wiring decision, `willreturn` audit, escape-analysis codegen. Docket **Qs.1** (`List<Int>` indexing) carries forward. **Next: v4.110.0 final Phase C release — re-run everything post-v4.108.0 and publish comprehensive Phase C deltas. Phase D (self-hosted maturity, docket cleanup) opens after.**)
