...
## Where We Are (v4.118.0 **Phase F release 1 — final cross-language benchmark.** The v4.120.0 panel's evidence document now exists. Zero compiler/runtime code changes; four version-string edits to the benchmark harness. All 6 workloads × 6 language configs × 10 runs ran to completion: **36/36 correct checksums, zero regressions**; raw data committed at `benchmarks/cross_language/v4.118.0-results.json`. Plus 5 native-async workloads × 3 languages × 10 runs at `benchmarks/async/v4.118.0-async.json` — first time this file has working Mapanare numbers after v4.94.0 had to skip the Mapanare side with "linking currently fails." **Headlines** (Mapanare O2 geomean across 6 workloads = 3.07 ms): **5.46× slower than C gcc -O2** (down from v4.107.0's 9.5× — a 2× narrowing, entirely from Phase C's v4.108.0 string_concat fix); **1.13× slower than Rust**; **on par with Go** (1.04×); **36.9× faster than Python 3.12**. **Async geomean 2.13 ms**: **42.6× faster than Python asyncio**, **1.74× slower than Go goroutines**. **Progress (Mapanare O2 v4.82.0 → v4.118.0)**: `string_concat` **102.31 → 1.32 ms (77.5× speedup)** — the one real win from the Phase A–E recovery arc; every other workload within ±10% once harness methodology (`/usr/bin/time -v` spawn overhead) is normalised. **`benchmarks/FINAL_REPORT_v4.120.md`** (500 lines) — 7 tables (wall / memory / binary / LOC / speedup vs C / progress / async), 6 per-workload ASCII position charts, methodology with toolchain versions (gcc 13.3, clang 18.1, rustc 1.94.1, go 1.22.5, python 3.12, LLVM 18.1.3), spectrum analysis by workload category, reproducibility checklist. `libmapanare_rt.a` byte-identical to v4.117.0. **No new dockets; no CARRY_FORWARD closures** (measurement-only). **Next: v4.119.0 writes the retrospective — full v4.0.0 → v4.118.0 journey, compiled statistics, v5 readiness pre-panel audit. v4.120.0 is the panel — 7 reviewers, v5 gate attempt 2.**)
