...
## Where We Are (v4.116.0 **Phase E release 2 — documentation batch.** Zero code changes. Closes five documentation gaps flagged by Boa (and others) since v4.82.0: README.md (version badge, headline benchmark line linking to PHASE_C_RESULTS, self-hosted LOC 15K → 38K, async row in Feature Status, async example in The Language section, stale "Coming in v4.2" corrected, Roadmap table extended); SPEC.md (header 1.0.0 Final → 4.116.0 Live + sync-discipline note, §29 v4.115.0 status paragraph documenting cooperative model and native I/O demo, §29.7 for-await reflagged as planned); docs/cookbook/async.md (corrected `mnc run` mischaracterization, §8 native compilation workflow, §9 file I/O example, §10 HTTP GET example, §11 Sh.9a/Sh.9b recipes); docs/guides/debugging.md (full rewrite — DWARF deferred to v5.x per §21.3, new focus on valgrind/ASan/TSan/ir_doctor/Culebra/integration harness); docs/guides/getting_started.md (NEW — 244 lines, practical build walk for compiled-languages developers). VERIFICATION.md documents 7/7 compile-and-run snippets PASS, 3/3 async goldens regression-clean (42/43/110 unchanged). `libmapanare_rt.a` byte-identical to v4.115.0. **No new dockets opened; all v4.115.0 dockets (Sh.9a, Sh.9b, Sh.10) remain open — now documented as user-facing recipes.** **Next: v4.117.0 is test-suite hardening — ASan CI gate, TSan CI gate, flaky audit, coverage report.**)
