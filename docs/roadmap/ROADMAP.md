...
## Where We Are (v4.121.0 **Phase F closeout release 1 — DWARF deferral warning + bounded-generic trait fix.** Closes the last 22 of the v4.117.0 flaky audit's deterministic test failures. Two surgical compiler edits: `mapanare/cli.py::_resolve_debug` restores the v4.29.0 stderr deferral warning that v4.62.0 silently removed (`-g` is a no-op until DWARF lands in v5.x per SPEC §21.3); `mapanare/lower.py` gains `_type_params_used_in_signature` so a function with declared but unused trait-bounded type params (e.g., `fn max<T: Ord>(a: Int, b: Int) -> Int`) is lowered as a regular non-generic instead of being deferred to a monomorphization that no caller could trigger. Plus the test hygiene v4.120.0's panel-only release skipped: 4 stale-assertion failures re-pinned at `-O0` (the inliner had grown teeth and was eliminating IR shapes the assertions counted), 14 stale CLI tests retired or rewritten against the surviving `build` subcommand (the `compile` Python emitter was removed in v3.x). **v4.117.0 audit subset (1,501 tests across 9 subdirectories): 0 failures across 3 sequential runs.** Full `pytest tests/` still shows 51 failures outside the audit scope — that is panel item An.1 and out of v4.121.0 scope. Lint debt unchanged (panel item An.2). `libmapanare_rt.a` byte-identical to v4.120.0. **Next: v4.122.0 fixes Qs.1 (`List<Int>` indexing in argument position).**)
