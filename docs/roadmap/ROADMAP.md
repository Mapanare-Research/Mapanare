...
## Where We Are (v4.120.0 **Phase F panel — v5 gate attempt 2 → Option B.** Seven reviewers graded the v4.100.0-v4.119.0 recovery arc. **Aggregate 8.21/10** (identical to v4.114.0 Phase D panel). **Verdict: 2 PASS (Boa 8.7 docs, Mamba 8.5 perf) + 4 PASS WITH NOTES (Rattler 8.3, Viper 8.4, Cobra 7.9, Coral 8.1) + 1 NEEDS WORK (Anaconda 7.6 CI/testing)**. Mechanical rule fires Option B (aggregate < 9.0 AND any NEEDS WORK). Lead independently chose Option B. **v5.0.0 NOT tagged.** The load-bearing panel finding: the full pytest suite shows 73 failures and `make lint` shows 302 findings — the v4.117.0 flaky audit measured only a subset (9 subdirectories, 1,501 tests), and 51 failures live outside that scope. Qs.1 (`List<Int>` indexing in argument position prints `<?>` through native pipeline) reproduced fresh by 3 reviewers. 17 carry-forward items opened. Proposed 6-release closeout arc ending at v4.130.0 v5 gate attempt 3 (v4.121.0 test+lint hygiene → v4.122.0 Qs.1 + DWARF → v4.123.0 Rt.1 unbox → v4.124.0 Sh.8 constructor → v4.125.0 refresh → v4.126.0 dead-code sweep → buffer → panel). Panel score trajectory v4.99.0 → v4.106.0 → v4.114.0 → v4.120.0: **6.59 → 7.87 → 8.21 → 8.21**. `libmapanare_rt.a` byte-identical to v4.119.0. Zero compiler/runtime code changes. **Next: v4.121.0 opens — test + lint hygiene sweep per `docs/roadmap/v4/v4.121.0/PLAN.md`.**)
