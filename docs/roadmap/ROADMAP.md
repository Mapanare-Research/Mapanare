...
## Where We Are (v5.3.3 **SPEC + docs polish — last release before v5.4.0 re-panel.** Zero compiler or runtime source changes. Closes Coral's three open LOW-severity carry-forwards from the v5.3.0 panel: **SPEC-pkg** (new §30 Package Management — `mapanare.toml` manifest schema with required/optional fields, `[dependencies]` and `[dev-dependencies]` tables with inline git specs, version constraint grammar `^`/`~`/`>=`/exact/`*`, `mapanare install` semantics as a 7-step pipeline with SHA-256 integrity check + atomic extract to `mn_modules/<name>-<version>/`, `mapanare.lock` JSON schema with `lockfile_version: 1` + integrity hashes, registry API surface at `https://mapanare.dev` with five GET endpoints + one POST publish endpoint, GitHub OAuth + `~/.mapanare/token` auth flow, six-bullet security model including the **no install-time scripts** rule, and §30.7 Out-of-Scope listing what v5.x does NOT specify — transitive resolution, yanking, private registries, vendoring, signatures beyond SHA-256, offline mirrors). **SPEC header** (bumped 4.143.0 → 5.3.3, sync date 2026-04-18 → 2026-04-22) — 27-release staleness flagged by Coral at three consecutive panels finally closed. **Demo gap (signals)** (new `examples/signals/counter.mn` — standalone reactive counter showing `let mut count = signal(0)` + `let doubled = signal { count.value * 2 }` with `count.value = N` updates triggering automatic re-evaluation of `doubled.value`; header comments explain that the Python bootstrap's C backend currently has a signal-typing bug and route users to the LLVM compile path via `python -m mapanare emit-llvm ... -o counter.ll` + `clang counter.ll libmapanare_rt.a -o counter`). **Verification**: `pytest tests/test_spec.py` 45 passed; `black --check .` + `ruff check .` 0 errors; LLVM emit of the signal demo produces 20 `__mn_signal_*` calls in valid IR. No compiler, runtime, or self-hosted changes. **Arc close**: v5.3.x closeout arc (v5.3.1 MEDIUM cleanup + v5.3.2 stage2 inliner restore + v5.3.3 SPEC+demo) is complete. All 5 MEDIUMs + 3 LOWs from v5.3.0 either closed or explicitly deferred to v5.x feature track. **Next: v5.4.0 — RE-PANEL** (target 9.5+).)

## Where We Are (v5.3.2 **In.1-stage2 — restore fixed-point (clone_instr_for_inline).** Extends `clone_instr_for_inline` from 10 to all 38 Instruction enum variants. The v5.1.2 In.1 fix handled use-renaming for all variants but only cloned definitions for 10 — the remaining 28 fell through with un-renamed destinations, causing `llvm-as` rejection of stage2.ll (`use of undefined value '%_inl0_6_t4'`). Three additional fixes: `rename_phi_pred` helper for PHI predecessor labels after block splitting; param-count guard for `new_decorator` name collision; downstream PHI predecessor update from original block to merge block. **Fixed-point: BROKEN → stage2 llvm-as OK.** stage3 blocked by MIR verifier in stage2 binary (Ve.1 LOW — bootstrapping divergence). 54/66 goldens. 9 inline rename tests (5 new). Closes In.1-stage2 MEDIUM. Opens Ve.1 LOW. **Next: v5.3.3.**)

## Where We Are (v5.3.1 **Quick-win closeout — lint GREEN, stream-C fixed, docs accurate.** First release of the v5.3.x closeout arc targeting v5.4.0 re-panel at 9.5+. Closes 5 MEDIUM + 3 LOW carry-forwards from the v5.3.0 panel. **Lint-v5.2.0** (MEDIUM): `black --check . && ruff check .` returns 0 (was already green). **Bo.15** (MEDIUM): README fixed-point claim qualified — "strict 3-stage fixed point" was stale since v5.1.2 In.1 inliner re-enable; now accurately reflects regression + restoration track. **Bo.16** (MEDIUM): `docs/known_issues.md` updated — removed "No package manager yet"; added v5.2.0 registry entry. **Stream-C** (MEDIUM): 4 stream tests in `tests/native/test_c_runtime.c` fixed — `MnList list = {0}` (sets `elem_size = 0`, triggering Ge.1r silent 256B fallback) replaced with `__mn_list_new(sizeof(int64_t))`. All 3 C hardening classes pass (Plain, ASan, TSan). **An.9r** (LOW): `test_post_opt_single_switch_in_hot_loop` accepts 0 or 1 switches (LLVM 18 folds further). **Bo.17** (LOW): zh-CN/pt/es README badges bumped to 5.3.1. **Bo.14r** (LOW): `getting_started.md` version and test count refreshed. No compiler or runtime source changes. Verification: 5462 passed / 2 failed (version-mismatch only) / 116 skipped. Expected panel impact: Anaconda +0.3-0.5, Boa +0.1-0.2, Mamba +0.05. **Next: v5.3.2.**)

## Where We Are (v5.3.0 **THE PANEL — 9.30/10, Option A.** Seven-reviewer panel grading the v5.0.1-v5.2.0 arc (12 releases, 21 commits, 19 carry-forward closures). **Aggregate: 9.30/10. Grade distribution: 5 EXCEEDS (Viper 9.7, Mamba 9.6, Coral 9.4, Boa 9.4, Rattler 9.3) / 2 MEETS (Anaconda 8.9, Cobra 8.8) / 0 NEEDS WORK.** Mechanical rule: aggregate >= 9.0 AND 0 NEEDS WORK -> **Option A**. v5.3.0 is a clean production release. Score trajectory: 6.59 -> 7.87 -> 8.21 -> 8.21 -> 8.80 -> 8.86 -> 9.21 -> 9.37 -> **9.30**. The -0.07 dip from v4.154.0 (9.37) comes from the In.1 inliner fixed-point regression (Cobra -0.3, Rattler -0.3) and v5.2.0 lint gap (Anaconda -0.5), partially offset by Perf.1/Perf.2 performance wins (Mamba +0.3), Ge.1r/Own.1 safety closures (Viper +0.1), and documentation improvements (Boa +0.1, Coral +0.1). Carry-forward: In.1-stage2 (MEDIUM, inliner handles 10/30 instruction kinds), lint fix (MEDIUM), stream-C tests (MEDIUM), README fixed-point claim (MEDIUM), known_issues.md stale (MEDIUM). **No compiler or runtime source changes — pure review.** Evidence: `docs/roadmap/v5/v5.3.0/MEASUREMENTS.md`. Reviews: `.reviews/v5.2.0/`. **Next: v5.4.0.**)

## Where We Are (v5.2.0 **Package Registry MVP — publish + install end-to-end.** First minor bump since v5.0.0 with a user-facing feature. Closes the last open item on V5_READINESS.md ("package registry infrastructure"). **Backend**: `mapanare.dev/api/packages` — FastAPI + PostgreSQL/SQLite, deployed on Render (matching existing `mapanare-website` infra). GitHub OAuth for auth; API tokens; download tracking. Team-only publishing for MVP; open publishing v5.3+. **Client** (`stdlib/pkg.py` + `mapanare/cli.py`): `mapanare.toml` gains `repository` field; `mapanare install foo@1.2.3` downloads from registry, verifies SHA-256, extracts to `mn_modules/foo-1.2.3/`; `mapanare install` (no args) resolves all `[dependencies]` from `mapanare.toml`; `mapanare publish` uploads tarball with metadata fields to `/api/packages`; `mapanare.lock` records resolved versions deterministically. Install directory renamed `mapanare_packages/` → `mn_modules/` with versioned subdirectories. Semver resolution: latest-satisfying with `^`, `~`, `>=`, exact, `*` — no SAT solver. **No install-time scripts allowed** (supply-chain discipline). **Tests**: `tests/registry/` — 51 tests covering manifest parsing, lockfile round-trip, semver resolution, tarball creation, integrity hashing, install-all-from-manifest, and registry URL/path assertions. **Docs**: `docs/guides/packages.md` — user-facing guide for publish, install, `mapanare.toml` schema, lockfile, auth flow. **Deferred**: open publishing, peer-dep resolution, yanking, vendoring (all v5.3+ scope). **Next: v5.3.0.**)

## Where We Are (v5.1.4 **Perf.2 — lazy thread creation in coro scheduler.** `__mn_coro_scheduler_init` now pre-creates 2 workers eagerly (worker 0 = caller, worker 1 = 1 spawned thread) and grows the pool lazily when `active_tasks > workers * 8`, up to the cpu-count cap. Idle workers self-exit after 100 ms when pool has > 2 workers. Race-safe teardown uses per-worker `spawned[]` + `worker_exited[]` arrays; `spawn_lock` mutex guards spawn/exit mutations. Default-settings async geomean: **2.3 → 1.19 ms (0.91× Go)** — the "faster than Go" headline now holds at default settings without `MAPANARE_ASYNC_THREADS`. Env var preserved as optional override for power users. TSan 0 races; valgrind 0 leaks; 54/66 goldens unchanged. PARITY_GAPS.md: Perf.2 → Historical. Mamba's v4.154.0 "not a portable claim" complaint is closed. **Next: v5.2.0.**)

## Where We Are (v5.1.3 **Own.1 Phase 1 — compiler-enforced drop-glue skip on ownership transfer.** Applies the Cb.7 zero-after-push workaround to `register_struct` (lower.mn:330-336) and `register_enum` (lower.mn:364-369) — the two sites Viper has flagged every panel since v4.99.0 (28 releases open). Design finding: the self-hosted emitter (`emit_llvm.mn`) has NO `emit_drop_glue` function — no epilogue cleanup, no ownership tracking, no `_move_resource`. The Python emitter already handles this correctly via `_do_call`'s blanket move-on-call (line 3882). The full Move instruction + `@takes_ownership` annotation + `moved_locals` infrastructure deferred to v5.1.4+ (requires drop-glue emission in self-hosted emitter first — estimated 200-400 lines). `DESIGN.md` documents the annotation-vs-inference decision and the full deferred design. Valgrind: 0 ERRORS across 66 goldens (unchanged). Golden tests: 54/66 (unchanged). Stage2.ll: +25 lines (+5 `__mn_list_new` calls from zeroed locals). PARITY_GAPS.md: Own.1 Phase 1 moves to Historical; Phase 2 (Move instruction + drop-glue) remains open for v5.1.4+; full borrow checker v6.0. **Next: v5.1.4.**)

## Where We Are (v5.1.0 **Inline list ops in LLVM IR — Perf.1 closed.** Replace opaque `call @__mn_list_get` / `@__mn_list_set` with inline `getelementptr` + `load`/`store` for value-type lists (`elem_size == 8`: `List<Int>`, `List<Float>`, `List<Ptr>`). LLVM can now see through to the backing buffer, enabling SROA, vectorization, and loop hoisting across list access. Inline bounds check emitted with `abort()` trap; LLVM's ScalarEvolution folds it in loops where the index is proven in-range. Fallback to opaque call for String-element lists (16B) and structs. Python and self-hosted emitters patched simultaneously — no new parity gap. **quicksort: 2.99× → 1.14× Rust (62% improvement). Geomean: 1.30× → 1.10× Rust.** 10 new tests in `tests/llvm/test_list_inline.py`. Golden tests 54/66 unchanged. Closes Perf.1 (Mamba v4.154.0). **Next: v5.1.1.**)
