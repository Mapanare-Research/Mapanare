...
| 3 | Verify `mnc-stage1` emits correct LLVM IR for simple programs | `[~]` | **IR generation verified correct** (2026-03-15): All key patterns (list concat in loops, list_push + FieldSet write-back, emit_instr shared-buffer IndexSet, lower_fn function-to-module chain) produce correct LLVM IR. elem_size correct for all list types (276 for Definition, 100 for Stmt, 64 for FnEntry, 16 for String). Debug prints removed from parser.mn, emit_llvm.mn, main.mn. **Remaining:** mnc-stage1 produces IR preamble but no function bodies at runtime — needs WSL/gdb debugging. Likely a runtime issue (ABI, stack, or memory) since the IR is structurally correct. |
...
| 2026-03-15 | 3 | Phase 3 Task 3: IR inspection + test hardening | Phase 3 Task 3 continued: WSL runtime debugging | **IR verified correct.** Inspected `main.ll` (55K lines) for all key patterns: `emit_line` (list push + FieldSet write-back), `emit_instr` (shared-buffer IndexSet), `lower_fn` (function-to-module write-back chain), `parser__parse` (defs concat in loop). All generate correct LLVM IR with proper alloca store-back, elem_size values, and cross-block loads. **Test fixes:** Updated `test_verification.py` to use MIR pipeline (was using AST emitter which lacks `.push()`). Added Windows skip markers for ELF binary tests. Removed 5 debug println statements from `parser.mn`, `emit_llvm.mn`, `main.mn`. 3603 tests pass, 47 skipped. **Remaining:** mnc-stage1 produces IR preamble but no function bodies at runtime. IR is correct → issue is runtime-only (ABI, stack overflow from deep recursion with huge structs, or memory corruption). **Next session on WSL:** (1) Rebuild mnc-stage1 with clean self-hosted sources. (2) Run `./mnc-stage1 test.mn 2>&1` and examine output. (3) Run under gdb: `gdb ./mnc-stage1` → `break emit_llvm__emit_mir_function` → check if module.functions is empty. (4) If empty, break on `lower__lower_fn` to check if functions are being added to module. (5) Try AddressSanitizer build (`scripts/build_stage1.py` with ASan flags) to catch memory bugs. |
