...
| 2026-03-14d | 2+3 | Enum sizing fixed (Bug #21) | Debug boxed enum field extraction in semantic checker | **Bug #21 fixed: Recursive enum infinite sizing.** Recursive enums (Expr contains Binary(Expr, String, Expr), ElseClause contains ElseIf(Expr, Block, Option<ElseClause>)) had infinite type sizes — two-pass registration never converged. **Fix:** Auto-boxing recursive enum fields as heap-allocated pointers. Three mechanisms: (1) `_is_self_ref_field` detects direct self-recursion (A→A). (2) Iterative convergence loop in `emit()` detects mutual recursion (A→B→A) by comparing sizes between passes. (3) `_box_mutual_recursion` marks cross-references as boxed. `_emit_enum_init` calls `malloc()` for boxed fields, stores pointer in payload. `_emit_enum_payload` dereferences pointer to load value. **Results:** Expr: 588→84 bytes, Stmt: 716→100, Pattern: 592→52. Binary 15MB→8.5MB. 350/350 LLVM tests pass. Converges in 3 passes. **Remaining crash:** Same location as before fix: `semantic__check_call_resolved` → `__mn_str_eq` with null pointer. ASan: SEGV on 0x0. Pattern match `Ident(name)` extracts null string. The boxing itself works (72 malloc calls, correct types), so the null string comes from somewhere upstream. **Investigation for next session:** (1) Check if the Expr value arriving at `check_call_resolved` was correctly constructed by the parser — dump tag and first 16 payload bytes. (2) Verify that unboxing in `_emit_enum_payload` produces the correct Expr value when extracting callee from `Call(callee, args)`. (3) Check if the issue is in how the parser creates `Ident(name)` nodes — does the String get stored correctly in the payload? (4) Look at the generated IR for `parser__parse_call_expr` or similar to verify the enum init and enum payload instructions are correct. (5) The previous session's note about tokenize() return value ABI mismatch may still be relevant — the parser may be producing corrupt AST. **Key files:** `emit_llvm_mir.py` (all changes: `_is_self_ref_field`, `_field_refs_enum_in_set`, `_box_mutual_recursion`, modified `_register_enum`, `_emit_enum_init`, `_emit_enum_payload`, `_resolve_enum_name`, `_rt_malloc`), `mapanare/self/semantic.mn:768` (crash location), `mapanare/self/parser.mn` (AST construction), `mapanare/self/ast.mn:58` (Expr enum definition). |
