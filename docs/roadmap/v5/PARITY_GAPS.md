...
| **DX.5** (BREAKING) | `mnc <file.mn>` now compiles and runs the program — the implicit-run shape that newcomers expect. Pre-v5.9.1 the default was LLVM IR emission to stdout (useful for compiler devs, hostile first impression for everyone else). The IR-emission path moves to a new `mnc emit-llvm <file.mn> [-o output]` subcommand, parallel to the Python CLI's `mapanare emit-llvm`. `mn_main` gains an `emit-llvm` dispatch branch (between `cache` and `compile`); the new helper `run_emit_llvm(filename: String)` lifts the pre-v5.9.1 default-fall-through body and adds optional `-o <path>` (writes via `__mn_file_write` instead of `print`). The default fall-through becomes: `.mn` files print a one-line stderr deprecation note (`note: 'mnc <file.mn>' now runs the program; use 'mnc emit-llvm' for IR output`) and route to `run_program`; non-`.mn` files error with a migration hint pointing at `mnc emit-llvm` (raw IR) or `mnc compile` (transpilation). The deprecation note is on stderr only — `mnc file.mn > out.ll` redirections aren't polluted; CI scripts that pipe `2>&1` see one extra log line per build. Note removed in v5.11.0; v5.10.0 keeps it. **Dispatch-layer only** — zero compiler internals touched; same scope discipline as v5.9.0. NO seed refresh required (no new builtin call sites). | **v5.9.1** | `tests/test_cli_default.py` — 6 cases: `test_default_runs_program` (stdout has program output, no IR markers), `test_default_prints_deprecation_note` (stderr has the migration hint), `test_emit_llvm_subcommand_to_stdout` (IR markers on stdout, no deprecation note on stderr), `test_emit_llvm_to_file` (`-o` writes file, stdout silent), `test_non_mn_file_errors_with_hint` (Makefile → "not a .mn file" + emit-llvm hint, exit≠0), `test_help_lists_emit_llvm` (`--help` lists the new subcommand). Manual smoke on Linux (mnc `/tmp/dx5.mn` → "hello v5.9.1"; `mnc emit-llvm` → 12 KB `.ll`). Strict 3-stage fixed-point preserved (the v5.9.0 milestone). Goldens 66/66 byte-identical. |
