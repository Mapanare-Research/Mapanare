...
| Ve.2 | **NEW v5.6.5** — `ListInit` MIR drops the source type annotation for empty-literal lists: `let xs: List<String> = []` lowers with `elem_ty.kind=TK_UNKNOWN`, which `resolve_mir_type` maps to `"i64"` → `emit_list_init` sees a scalar type and either (a) allocates 8-byte slots (truncates 16-byte Strings → corruption), or (b) falls back to a 384-byte floor (~24× memory overhead but correct). Had been masked since v4.x by an unconditional 384-byte floor; v5.6.5's GEP-trick refactor re-surfaced it as the remaining blocker for non-empty `stage3.ll`. Symptomatic crash: `mnc-stage2` on non-trivial programs (e.g. `fn add(a,b){return a+b} fn main(){print(add(1,2))}`) OOMs with a garbage size request or SIGSEGVs in `__mn_str_concat`. Fix: `lower.mn::ListInit` should carry the `let`-declaration type annotation. v5.6.7 scope (~1 session). | use `mnc-stage1` for self-host work; `verify_fixed_point.sh` still fails at Stage 2 | v5.6.7 |
