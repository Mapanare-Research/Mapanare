...
Last updated: v5.6.8 (Ve.3 investigation-only release; ships unchanged source vs v5.6.7).
...
| Ve.3 | **INVESTIGATION-ONLY v5.6.8** — `mnc-stage2` on non-trivial programs (e.g. `fn add(a,b){return a+b} fn main(){print(add(1,2))}`) OOMs with a garbage-size request: `__mn_alloc` receives a non-deterministic huge value (5×10^18 / 6.6×10^18 / etc. — varies across runs). Stack: `__mn_alloc → __mn_str_concat → llvm_alloca → emit_mir_by_kind`. v5.6.8 narrowed the failure: the corrupt input is the `name` parameter into `llvm_alloca`, derived from `dest.name` of a `Value` extracted via `instr_dest(inst)` for the **third** `Instruction::Alloca` emitted (the first two — `%a.addr` / `%b.addr` for `add`'s param slots — emit correctly). `dest.ty` reads correctly on all three calls (kind=TK_INT, name="Int"); only `dest.name` (Value's field 0) is corrupt on the third — len reads as a non-deterministic huge value, indicating uninitialised memory. PLAN's Hypothesis A (payload-type builder divergence) ruled out by IR inspection; Hypothesis D (Python emitter bug) ruled out by `mnc-stage1-fromself` reproducing the OOM. Adjacent finding: `struct_byte_size` undercounts named structs at 24 (not 80) for `%struct.Value` due to `llvm_aggregate_size` counting commas inside nested aggregates; an experimental patch (delegate to `llvm_sizeof_st`) was tried and DID NOT close the OOM (stage2.ll +7%, dest.name still corrupt on third call). Patch reverted; bug is unrelated to ABI sizing. Active hypothesis for v5.6.9+: the inliner (`mir_opt::clone_instr_for_inline`) or lowerer creates the third Alloca with corrupt dest.name, OR Value's field-0 boundary has a layout mismatch the Python emitter avoids (Python emits `noalias` on byref params and inline aggregate types; self-hosted emitter does not). See `docs/roadmap/v5/v5.6.8/SESSION_REPORT.md` for the full investigation log. Persists since v5.4.4. | use `mnc-stage1` for self-host work; `verify_fixed_point.sh` fails at Stage 2 | v5.6.9+ |
