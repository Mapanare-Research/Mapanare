...
def _redis_source() -> str:
    """Return inlined KV + Redis module sources."""
    return _KV_MN + "\n\n" + _strip_imports(_REDIS_MN)
...
class TestRedisConnect:
    def test_connect_compiles(self) -> None:
        ...
        src = _redis_source_with_main("""\
            let r: Result<KVStore, KVError> = connect("127.0.0.1", 6379)
            match r {
        ...
        src = _redis_source_with_main("""\
            ...
            match r {
                Ok(store) => {
                    match store {
                        Redis(rs) => {
                            let sr: Result<Bool, KVError> = set(rs, "__mn_test_key", "test_value")
                            let val: Option<String> = get(rs, "__mn_test_key")
                            match val {
                                Some(v) => { println(v) },
        ...
        src = _redis_source_with_main("""\
            ...
            match r {
                Ok(store) => {
                    match store {
                        Redis(rs) => {
                            let sr: Result<Bool, KVError> = set(rs, "__mn_test_del", "to_delete")
                            let dr: Result<Bool, KVError> = del(rs, "__mn_test_del")
                            match dr {
        ...
        src = _redis_source_with_main("""\
            ...
            match r {
                Ok(store) => {
                    match store {
                        Redis(rs) => {
                            let sr: Result<Bool, KVError> = set(rs, "__mn_test_exists", "val")
                            let found: Bool = exists(rs, "__mn_test_exists")
                            println(str(found))
                        },
                        Embedded(es) => { println("wrong variant") }
                    }
        ...
        src = _redis_source_with_main("""\
            ...
            match r {
                Ok(store) => {
                    match store {
                        Redis(rs) => {
                            let ir: Result<Int, KVError> = incr(rs, "__mn_test_counter")
                            match ir {
                                Ok(v) => { println(str(v)) },
                                Err(e) => { println(e.message) }
                            }
        ...
        src = _redis_source_with_main("""\
            ...
            match r {
                Ok(store) => {
                    match store {
                        Redis(rs) => {
                            let dr: Result<Int, KVError> = decr(rs, "__mn_test_counter")
                            match dr {
                                Ok(v) => { println(str(v)) },
                                Err(e) => { println(e.message) }
                            }
        ...
        src = _redis_source_with_main("""\
            ...
            match r {
                Ok(store) => {
                    match store {
                        Redis(rs) => {
                            let sr: Result<Bool, KVError> = set(rs, "__mn_keys_a", "1")
                            let sr2: Result<Bool, KVError> = set(rs, "__mn_keys_b", "2")
                            let matched: List<String> = keys(rs, "__mn_keys_*")
        ...
        src = _redis_source_with_main("""\
            ...
            match r {
                Ok(store) => {
                    match store {
                        Redis(rs) => {
                            let cr: Result<Bool, KVError> = close(rs)
                            match cr {
                                Ok(b) => { println("closed") },
                                Err(e) => { println(e.message) }
                            }
