...
def test_step_3_non_multiple(runtime_archive: Path, clang_bin: str, tmp_path: Path) -> None:
...
def test_step_zero_literal_rejected_at_lower(
    ...
    src = textwrap.dedent("""\
        fn main():
            ...
            print(str(tensor_size(s)))
        """)
    out, err, rc = _emit_link_run(
        src, runtime_archive, clang_bin, tmp_path, expect_compile_error=True
    )
    assert rc != 0
...
def test_step_slice_ir_shape_no_noalias(runtime_archive: Path, tmp_path: Path) -> None:
    ...
    assert (
        "noalias ptr @__mn_tensor_step_slice" not in ir
    ), "step_slice declaration must not carry noalias"
    for line in ir.splitlines():
        if "@__mn_tensor_step_slice" in line and "call" in line:
            assert "noalias" not in line, f"step_slice call must not carry noalias: {line!r}"
