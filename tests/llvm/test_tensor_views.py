...
def test_view_ir_no_noalias(runtime_archive: Path, tmp_path: Path) -> None:
    ...
    assert "noalias ptr @__mn_tensor_view" not in ir, (
        "view declaration must not carry noalias — view aliases parent's data buffer"
    )
    assert "noalias ptr @__mn_tensor_reshape" not in ir, (
    ...
    for line in ir.splitlines():
        if "@__mn_tensor_view" in line and "call" in line:
            assert "noalias" not in line, (
                f"view call site must not carry noalias: {line!r}"
            )
        if "@__mn_tensor_reshape" in line and "call" in line:
            assert "noalias" not in line, (
                f"reshape call site must not carry noalias: {line!r}"
            )


def test_view_size_mismatch_aborts(
    runtime_archive: Path, clang_bin: str, tmp_path: Path
) -> None:
    ...
    assert "tensor view" in result.stderr.lower(), (
        f"expected structured tensor-view error in stderr; got: {result.stderr!r}"
    )
    assert "size 4" in result.stderr and "size 9" in result.stderr, (
